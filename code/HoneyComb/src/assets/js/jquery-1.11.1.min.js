...
(function($){var _PLUGIN_='mmenu',_VERSION_='5.5.3';if($[_PLUGIN_]&&$[_PLUGIN_].version>_VERSION_){return;}
	$[_PLUGIN_]=function($menu,opts,conf){this.$menu=$menu;this._api=['bind','init','update','setSelected','getInstance','openPanel','closePanel','closeAllPanels'];this.opts=opts;this.conf=conf;this.vars={};this.cbck={};if(typeof this.___deprecated=='function'){this.___deprecated();}
		...
		return this;};$[_PLUGIN_].version=_VERSION_;$[_PLUGIN_].addons={};$[_PLUGIN_].uniqueId=0;$[_PLUGIN_].defaults={extensions:[],navbar:{add:true,title:'Menu',titleLink:'panel'},onClick:{setSelected:true},slidingSubmenus:true};$[_PLUGIN_].configuration={classNames:{divider:'Divider',inset:'Inset',panel:'Panel',selected:'Selected',spacer:'Spacer',vertical:'Vertical'},clone:false,openingInterval:25,panelNodetype:'ul, ol, div',transitionDuration:400};$[_PLUGIN_].prototype={init:function($panels){$panels=$panels.not('.'+_c.nopanel);$panels=this._initPanels($panels);this.trigger('init',$panels);this.trigger('update');},update:function(){this.trigger('update');},setSelected:function($li){this.$menu.find('.'+_c.listview).children().removeClass(_c.selected);$li.addClass(_c.selected);this.trigger('setSelected',$li);},openPanel:function($panel){var $l=$panel.parent();if($l.hasClass(_c.vertical)){var $sub=$l.parents('.'+_c.subopened);if($sub.length){return this.openPanel($sub.first());}
		$l.addClass(_c.opened);}
	else{if($panel.hasClass(_c.current)){return;}
		...
		if($panel.hasClass(_c.opened)){$panel.nextAll('.'+_c.opened).addClass(_c.highest).removeClass(_c.opened).removeClass(_c.subopened);}
		else{$panel.addClass(_c.highest);$current.addClass(_c.subopened);}
		...
		$oldpanels.each(function(){var $t=$(this),$p=$t;if($t.is('ul, ol')){$t.wrap('<div class="'+_c.panel+'" />');$p=$t.parent();}
		else{$p.addClass(_c.panel);}
			...
			$curpanels=$curpanels.add($p);});var $allpanels=$('.'+_c.panel,this.$menu);$curpanels.each(function(i){var $t=$(this),$p=$t.parent(),$a=$p.children('a, span').first();$text=$a.text();if(!$p.is('.'+_c.panels)){$p.data(_d.sub,$t);$t.data(_d.parent,$p);}
			if(!$p.children('.'+_c.next).length){if($p.parent().is('.'+_c.listview)){if($.browser.msie&&$.browser.version=="8.0"){var id=$t.attr('id'),$b=$('<a class="'+_c.next+'" href="#'+id+'" data-target="#'+id+'" >'+$text+'</a>').insertBefore($a);}else{var id=$t.attr('id'),$b=$('<a class="'+_c.next+'" href="#'+id+'" data-target="#'+id+'" />').insertBefore($a);}
				if($a.is('span')){$b.addClass(_c.fullsubopen);}}}
			if(!$t.children('.'+_c.navbar).length){if(!$p.hasClass(_c.vertical)){if($p.parent().is('.'+_c.listview)){var $p=$p.closest('.'+_c.panel);}
			else{var $a=$p.closest('.'+_c.panel).find('a[href="#'+$t.attr('id')+'"]').first(),$p=$a.closest('.'+_c.panel);}
				var $navbar=$('<div class="'+_c.navbar+'" />');if($p.length){var id=$p.attr('id');switch(that.opts.navbar.titleLink){case'anchor':_url=$a.attr('href');break;case'panel':case'parent':_url='#'+id;break;case'none':default:_url=false;break;}
					$navbar.append('<a class="'+_c.btn+' '+_c.prev+'" href="#'+id+'" data-target="#'+id+'" />').append($('<a class="'+_c.title+'"'+(_url?' href="'+_url+'"':'')+' />').text($a.text())).prependTo($t);if(that.opts.navbar.add){$t.addClass(_c.hasnavbar);}}
				else if(that.opts.navbar.title){$navbar.append('<a class="'+_c.title+'">'+that.opts.navbar.title+'</a>').prependTo($t);if(that.opts.navbar.add){$t.addClass(_c.hasnavbar);}}}}});var $s=this.__findAddBack($panels,'.'+_c.listview).children('.'+_c.selected).removeClass(_c.selected).last().addClass(_c.selected);$s.add($s.parentsUntil('.'+_c.menu,'li')).filter('.'+_c.vertical).addClass(_c.opened).end().not('.'+_c.vertical).each(function(){$(this).parentsUntil('.'+_c.menu,'.'+_c.panel).not('.'+_c.vertical).first().addClass(_c.opened).parentsUntil('.'+_c.menu,'.'+_c.panel).not('.'+_c.vertical).first().addClass(_c.opened).addClass(_c.subopened);});$s.children('.'+_c.panel).not('.'+_c.vertical).addClass(_c.opened).parentsUntil('.'+_c.menu,'.'+_c.panel).not('.'+_c.vertical).first().addClass(_c.opened).addClass(_c.subopened);var $current=$allpanels.filter('.'+_c.opened);if(!$current.length){$current=$curpanels.first();}
		$current.addClass(_c.opened).last().addClass(_c.current);$curpanels.not('.'+_c.vertical).not($current.last()).addClass(_c.hidden).end().appendTo(this.$pnls);return $curpanels;},_initAnchors:function(){var that=this;glbl.$body.on(_e.click+'-oncanvas','a[href]',function(e){var $t=$(this),fired=false,inMenu=that.$menu.find($t).length;for(var a in $[_PLUGIN_].addons){if(fired=$[_PLUGIN_].addons[a].clickAnchor.call(that,$t,inMenu)){break;}}
		if(!fired&&inMenu){var _h=$t.attr('href');if(_h.length>1&&_h.slice(0,1)=='#'){try{var $h=$(_h,that.$menu);if($h.is('.'+_c.panel)){fired=true;that[$t.parent().hasClass(_c.vertical)?'togglePanel':'openPanel']($h);}}
		catch(err){}}}
		...
		return false;})()};var _c,_d,_e,glbl;function initPlugin(){if($[_PLUGIN_].glbl){return;}
	...
	var that=this;this.__transitionend(glbl.$page.first(),function(){that.$menu.removeClass(_c.current).removeClass(_c.opened);glbl.$html.removeClass(_c.opened).removeClass(_c.blocking).removeClass(_c.modal).removeClass(_c.background).removeClass(_c.mm(that.opts[_ADDON_].position)).removeClass(_c.mm(that.opts[_ADDON_].zposition));if(that.opts.extensions){glbl.$html.removeClass(that.opts.extensions);}
		glbl.$page.each(function(){$(this).attr('style',$(this).data(_d.style));});that.vars.opened=false;that.trigger('closed');},this.conf.transitionDuration);glbl.$html.removeClass(_c.opening);this.trigger('close');this.trigger('closing');};$[_PLUGIN_].prototype.closeAllOthers=function(){glbl.$allMenus.not(this.$menu).each(function(){var api=$(this).data(_PLUGIN_);if(api&&api.close){api.close();}});}
	$[_PLUGIN_].prototype.setPage=function($page){var that=this,conf=this.conf[_ADDON_];if(!$page||!$page.length){if(glbl.$body.find("> form").length>0){conf.pageSelector="> form";}
		$page=glbl.$body.find(conf.pageSelector);if(conf.noPageSelector.length){$page=$page.not(conf.noPageSelector.join(', '));}
		if($page.length>1&&conf.wrapPageIfNeeded||glbl.$body.find("> form").length>0){$page=$page.wrapAll('<'+this.conf[_ADDON_].pageNodetype+' />').parent();}}
		...
		glbl.$blck.appendTo(glbl.$body).off(_e.touchstart+'-'+_ADDON_+' '+_e.touchmove+'-'+_ADDON_).on(_e.touchstart+'-'+_ADDON_+' '+_e.touchmove+'-'+_ADDON_,function(e){e.preventDefault();e.stopPropagation();glbl.$blck.trigger(_e.mousedown+'-'+_ADDON_);}).off(_e.mousedown+'-'+_ADDON_).on(_e.mousedown+'-'+_ADDON_,function(e){e.preventDefault();if(!glbl.$html.hasClass(_c.modal)){that.closeAllOthers();that.close();}});};var _c,_d,_e,glbl;})(jQuery);(function($){var _PLUGIN_='mmenu',_ADDON_='navbars';$[_PLUGIN_].addons[_ADDON_]={setup:function(){var that=this,navs=this.opts[_ADDON_],conf=this.conf[_ADDON_];glbl=$[_PLUGIN_].glbl;if(typeof navs=='undefined'){return;}
	if(!(navs instanceof Array)){navs=[navs];}
	var _pos={};$.each(navs,function(n){var opts=navs[n];if(typeof opts=='boolean'&&opts){opts={};}
		if(typeof opts!='object'){opts={};}
		if(typeof opts.content=='undefined'){opts.content=['prev','title'];}
		if(!(opts.content instanceof Array)){opts.content=[opts.content];}
		opts=$.extend(true,{},that.opts.navbar,opts);var poss=opts.position,hght=opts.height;if(typeof hght!='number'){hght=1;}
		hght=Math.min(4,Math.max(1,hght));if(poss!='bottom'){poss='top';}
		if(!_pos[poss]){_pos[poss]=0;}
		_pos[poss]++;var $navbar=$('<div />').addClass(_c.navbar+' '+
			_c.navbar+'-'+poss+' '+
			_c.navbar+'-'+poss+'-'+_pos[poss]+' '+
			_c.navbar+'-size-'+hght);_pos[poss]+=hght-1;for(var c=0,l=opts.content.length;c<l;c++){var ctnt=$[_PLUGIN_].addons[_ADDON_][opts.content[c]]||false;if(ctnt){ctnt.call(that,$navbar,opts,conf);}
		else{ctnt=opts.content[c];if(!(ctnt instanceof $)){ctnt=$(opts.content[c]);}
			ctnt.each(function(){$navbar.append($(this));});}}
		var _content=Math.ceil($navbar.children().not('.'+_c.btn).length/hght);if(_content>1){$navbar.addClass(_c.navbar+'-content-'+_content);}
		if($navbar.children('.'+_c.btn).length){$navbar.addClass(_c.hasbtns);}
		$navbar.prependTo(that.$menu);});for(var poss in _pos){that.$menu.addClass(_c.hasnavbar+'-'+poss+'-'+_pos[poss]);}},add:function(){_c=$[_PLUGIN_]._c;_d=$[_PLUGIN_]._d;_e=$[_PLUGIN_]._e;_c.add('close hasbtns');},clickAnchor:function($a,inMenu){}};$[_PLUGIN_].configuration[_ADDON_]={breadcrumbSeparator:'/'};$[_PLUGIN_].configuration.classNames[_ADDON_]={panelTitle:'Title',panelNext:'Next',panelPrev:'Prev'};var _c,_d,_e,glbl;})(jQuery);(function($){var _PLUGIN_='mmenu',_ADDON_='navbars',_CONTENT_='title';$[_PLUGIN_].addons[_ADDON_][_CONTENT_]=function($navbar,opts){var _c=$[_PLUGIN_]._c;$navbar.append('<a class="'+_c.title+'"></a>');var update=function($panel){$panel=$panel||this.$pnls.children('.'+_c.current);var $node=$navbar.find('.'+_c.title),$orgn=$panel.find('.'+this.conf.classNames[_ADDON_].panelTitle);if(!$orgn.length){$orgn=$panel.children('.'+_c.navbar).children('.'+_c.title);}
	var _url=$orgn.attr('href'),_txt=$orgn.html()||opts.title;$node[_url?'attr':'removeAttr']('href',_url);$node[_url||_txt?'removeClass':'addClass'](_c.hidden);$node.html(_txt);};this.bind('openPanel',update);this.bind('init',function(){update.call(this);});};})(jQuery);(function($){var _PLUGIN_='mmenu',_ADDON_='navbars',_CONTENT_='prev';$[_PLUGIN_].addons[_ADDON_][_CONTENT_]=function($navbar,opts){var _c=$[_PLUGIN_]._c;$navbar.append('<a class="'+_c.prev+' '+_c.btn+'" href="#"></a>');this.bind('init',function($panl){$panl.removeClass(_c.hasnavbar);});var update=function(){var $panl=this.$pnls.children('.'+_c.current);var $node=$navbar.find('.'+_c.prev),$orgn=$panl.find('.'+this.conf.classNames[_ADDON_].panelPrev);if(!$orgn.length){$orgn=$panl.children('.'+_c.navbar).children('.'+_c.prev);}
	var _url=$orgn.attr('href'),_txt=$orgn.html();$node[_url?'attr':'removeAttr']('href',_url);$node[_url||_txt?'removeClass':'addClass'](_c.hidden);$node.html(_txt);};this.bind('openPanel',update);this.bind('init',update);};})(jQuery);(function($){var _PLUGIN_='mmenu',_ADDON_='navbars',_CONTENT_='next';$[_PLUGIN_].addons[_ADDON_][_CONTENT_]=function($navbar,opts){var _c=$[_PLUGIN_]._c;$navbar.append('<a class="'+_c.next+' '+_c.btn+'" href="#"></a>');var update=function($panel){$panel=$panel||this.$pnls.children('.'+_c.current);var $node=$navbar.find('.'+_c.next),$orgn=$panel.find('.'+this.conf.classNames[_ADDON_].panelNext);var _url=$orgn.attr('href'),_txt=$orgn.html();$node[_url?'attr':'removeAttr']('href',_url);$node[_url||_txt?'removeClass':'addClass'](_c.hidden);$node.html(_txt);};this.bind('openPanel',update);this.bind('init',function(){update.call(this);});};})(jQuery);(function($){var _PLUGIN_='mmenu',_ADDON_='navbars',_CONTENT_='close';$[_PLUGIN_].addons[_ADDON_][_CONTENT_]=function($navbar,opts){var _c=$[_PLUGIN_]._c,glbl=$[_PLUGIN_].glbl;$navbar.append('<a class="'+_c.close+' '+_c.btn+'" href="#"></a>');var setPage=function($page){$navbar.find('.'+_c.close).attr('href','#'+$page.attr('id'));};setPage.call(this,glbl.$page);this.bind('setPage',setPage);};})(jQuery);